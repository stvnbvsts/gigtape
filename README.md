...
Gigtape creates private Spotify or Apple Music playlists from setlist.fm data.

Search for an artist or festival, preview the songs from recent setlists,
remove or add tracks, and create a private playlist with a direct link.
...
- Apple Music web playlist creation through MusicKit authorization
...
- Copy-only transfer from the authenticated user's Spotify playlists to Apple Music
- Explicit reporting for tracks the selected music service could not match
...
packages/
  ...
  adapters/
    ...
    applemusic/        Apple Music token signing, search, and playlist creation
    spotify/           Spotify OAuth, search, playlist reading, and playlist creation
```
...
| Apple Music MusicKit key | - | Required only for Apple Music web support |
...
Apple Music support is web-only. The API signs short-lived MusicKit developer
...
2. Click **Connect Spotify** or **Connect Apple Music** and complete consent in the browser.
3. Search for an artist.
4. Pick the correct artist result.
5. Preview the setlist, remove songs, or add manual tracks.
6. Create the private playlist.
7. Open the returned playlist link.
...
To copy one of your own Spotify playlists to Apple Music, open the transfer
tool, connect both services, select a Spotify playlist, preview its tracks, and
copy it to Apple Music. Transfer is copy-only: Gigtape never deletes, moves,
renames, or mutates the source Spotify playlist. The copy preserves title,
description, and track order for matched tracks.
...
| GET | `/auth/apple-music/developer-token` | Get a MusicKit developer token |
| POST | `/auth/apple-music/session` | Start an Apple Music web session from a Music User Token |
...
| GET | `/transfer/spotify/playlists` | List the authenticated Spotify user's own playlists |
| GET | `/transfer/spotify/playlists/:id` | Preview one Spotify playlist |
| POST | `/transfer/spotify-to-apple-music` | Copy one Spotify playlist to Apple Music |
...
| Apple Music unavailable | Missing MusicKit credentials | Set `APPLE_MUSIC_TEAM_ID`, `APPLE_MUSIC_KEY_ID`, and `APPLE_MUSIC_PRIVATE_KEY` |
...
- Per-artist festival mode creates playlists sequentially, so large lineups can
  take a while.
- Apple Music support is web-only; CLI playlist creation remains Spotify-only.
- Playlist transfer is copy-only and currently limited to the authenticated
  user's own Spotify playlists as the source.
...
Apple Music support now starts with web playlist creation and copy-only
Spotify-to-Apple Music transfer. Future phases may add more services,
bidirectional transfer, or persistent account storage.
